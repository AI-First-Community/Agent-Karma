...
## [0.45.0]
- **Project identity finalized for launch.** Publisher is now **`innovate-with-sanjeev`** and the canonical home is **`AI-First-Community/Agent-Karma-Public`** (open-source, Apache-2.0). Marketplace links, repository/issue URLs, the in-app "Learn more"/settings deep-links, and badges all point there. Maintainer identity (git author + `package.json` author) is **Sanjeev Azad · `subscribe.sanjeev@gmail.com`** (the org account); the Marketplace publisher is under `sanjeev.azad@gmail.com`. No behavior change. _(Note: the extension ID changes to `innovate-with-sanjeev.agent-karma`, so local data lives under a new storage path — a clean slate, expected pre-launch.)_
